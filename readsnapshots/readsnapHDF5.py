...
#GFM 
	      "GAGE":["GFM_StellarFormationTime",1],
              ...
              "GSPH":["GFM_StellarPhotometrics",4], #band luminosities: U, B, V, K
  ...
  if (multiple_files):	
	if (slabflag==False):
		...
		for num in range(0,filenum):
			...
			if (first):
				data = read_block_single_file(curfilename, block_name, dim2, parttype, no_mass_replicate, fill_block_name, slab_start, slab_len, verbose)
				dim1+=data.shape[0]
				ret_val = data
				first = False 
			else:	 
		...
		left=slab_len
			...
			nloc=head.npart[parttype]
	                if (nloc > off):
                	        ...
                	        if (first):
                        	        data = read_block_single_file(curfilename, block_name, dim2, parttype, no_mass_replicate, fill_block_name, slab_start=start, slab_len=count, verbose=verbose)
	                                dim1+=data.shape[0]
        	                        ret_val = data
                	                first = False
	                        else:
	...
	ret_val=read_block_single_file(curfilename, block_name, dim2, parttype, no_mass_replicate, fill_block_name, slab_start, slab_len, verbose)
