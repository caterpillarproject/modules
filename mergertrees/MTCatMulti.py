...
def convertmt_multiple(dir,version=2):
    """
    dir is the tree data directory
    filelist is the list of the tree_x_y_z.dat file names
    version is 2 for RC1, 3 for RC2 (only affects shape parameters and T/U)
    """
    import glob as glob
    #filelist = glob.glob(dir+'tree_*')
    files = glob.glob(dir+'tree_*')
    filelist = []
    for filei in files:
        filelist.append(filei.split("/")[-1])

    
    print "acting over these files"
    print filelist
...
def convertmt(dir,time_me=False,version=2,verbose=False,filenamein=None,filenameout=None,fileindexname=None):
    """
...
class MTCatalogue:
    ...
    def __init__(self,dir,haloids=[],version=2,numHosts=np.infty,indexbyrsid=False,verbose=False):
        ...
        self.scale_list = [] ## TEMPORARY KLUDGE TO AVOID SCALE_LIST ERRORS
